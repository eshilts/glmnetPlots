...
plot_cv_path <- function(cvg) {
  ...
  p <- ggplot(cv_path, aes(x=negative_log_lambda)) +
    ...
    geom_text(aes(y=cvup, label=type_label), data=min_1se, size=3.5, hjust=-0.25, vjust=-3) +
    geom_text(aes(y=cvup, label=dev.ratio_label), data=min_1se, size=3, hjust=-0.25, vjust=-2) +
    geom_text(aes(y=cvup, label=nzero_label), data=min_1se, size=3, hjust=-0.25, vjust=-0.75) +
    opts()
